...
//  WhilePhoning1
//
//  Created by luke luo on 2018/11/07.
...
import UIKit
import CoreMotion
import Network  // 替换为 Network.framework
...
class FilingAndStreamingViewController: UIViewController, UITextFieldDelegate {
    ...
    //Stream variables declaration
    static let bufferSize = 4096
    let port: NWEndpoint.Port = .init(integerLiteral: 1337)
    var listener: NWListener?
    var connections: [NWConnection] = []
    let socketQueue = DispatchQueue(label: "Socket Queue")
    ...
    func handleConnection(_ connection: NWConnection) {
        ...
        let data = self.streamValueMatrix.data(using: .utf8) ?? Data()
